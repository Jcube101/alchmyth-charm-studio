...
const bulkConfig = pricingConfig["bag-charm"];
const initialSelection: PricingSelection = { quantity: bulkConfig.moq, charmCount: 0, customBranding: false, delivery: "standard", sampleFirst: false };
...
export function BulkCalculator({ productName }: { productName: string }) {
  ...
  return <div className="border border-border bg-card p-5 sm:p-7">
    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4"><div className="min-w-0"><p className="text-xs font-semibold uppercase text-primary">Made for your moment</p><h2 className="mt-1 truncate font-display text-2xl font-medium text-primary">Build your bulk order</h2></div><Sparkles className="shrink-0 text-primary"/></div>
    <div className="mt-7 space-y-7">
      <fieldset><div className="flex items-center justify-between"><Label htmlFor="quantity">Quantity</Label><Input id="quantity" type="number" min={config.moq} max={config.maxQuantity} step={config.quantityStep} value={selection.quantity} onChange={(event) => updateQuantity(Number(event.target.value))} className="w-24 text-center"/></div><Slider aria-label="Bulk order quantity" className="mt-5" min={config.moq} max={config.maxQuantity} step={config.quantityStep} value={[selection.quantity]} onValueChange={([value]) => updateQuantity(value ?? config.moq)}/><div className="mt-2 flex justify-between text-xs text-muted-foreground"><span>{config.moq} minimum</span><span>{config.maxQuantity} units</span></div></fieldset>
      <fieldset><div className="flex items-center justify-between"><Label>Customised charms</Label><span className="text-sm font-semibold">{config.options.charms.find((item) => item.count === selection.charmCount)?.label}</span></div><Slider aria-label="Number of customised charms" className="mt-5" min={config.options.charms[0]?.count ?? 0} max={config.options.charms.at(-1)?.count ?? 0} step={1} value={[selection.charmCount]} onValueChange={([value]) => setSelection((current) => ({ ...current, charmCount: value ?? 0 }))}/><div className="mt-3 grid grid-cols-4 text-center text-[11px] text-muted-foreground">{config.options.charms.map((item) => <span key={item.count}>{item.count === 0 ? item.label : `+${formatINR(item.perUnit)}`}</span>)}</div></fieldset>
      <OptionSwitch label={config.options.branding.label} description={`e.g. a flag · +${formatINR(config.options.branding.perUnit)}/unit`} checked={selection.customBranding} onCheckedChange={(checked) => setSelection((current) => ({ ...current, customBranding: checked }))}/>
      <fieldset><Label>Delivery</Label><div className="mt-3 grid grid-cols-2 gap-2">{(["standard", "express"] as const).map((delivery) => <Button type="button" key={delivery} variant={selection.delivery === delivery ? "default" : "outline"} onClick={() => setSelection((current) => ({ ...current, delivery }))}>{delivery === "standard" ? "Standard" : `Express · +${Math.round((config.options.delivery.express - 1) * 100)}%`}</Button>)}</div></fieldset>
      <OptionSwitch label={config.options.sample.label} description={`One-time ${formatINR(config.options.sample.fee)} fee`} checked={selection.sampleFirst} onCheckedChange={(checked) => setSelection((current) => ({ ...current, sampleFirst: checked }))}/>
    </div>
    ...
    <div className="mt-6 border-y border-border bg-secondary p-4"><p className="text-sm font-semibold text-primary">Price breakdown</p><div className="mt-3 space-y-2">{result.lineItems.map((item) => <div className="flex justify-between text-sm" key={item.label}><span className="text-muted-foreground">{item.label}{item.kind === "per-unit" ? " / unit" : ""}</span><span>{item.amount < 0 ? "−" : "+"}{formatINR(Math.abs(item.amount))}</span></div>)}</div></div>
